            ...
            <div class="col-sm-6">
                <ul class="pagination">
                    <li v-bind:class="{disabled: current_page <= 1}">
                        <a href="#" v-on:click="change(1)">&laquo;</a>
                    </li>
                    <li v-bind:class="{disabled: current_page <= 1}">
                        <a href="#" v-on:click="change(current_page - 1)">&lt;</a>
                    </li>
                    <li v-for="page in pages" :key="page" :class="{active: page == current_page}">
                        <a href="#" v-on:click="change(page)">{{ page }}</a>
                    </li>
                    <li v-bind:class="{disabled: current_page >= last_page}">
                        <a href="#" v-on:click="change(current_page + 1)">&gt;</a>
                    </li>
                    <li v-bind:class="{disabled: current_page >= last_page}">
                        <a href="#" v-on:click="change(last_page)">&raquo;</a>
                    </li>
                </ul>
            </div>
...
export default {
    ...
    methods: {
        load(page) {
            axios.get('/api/user/list?page=' + page).then(({ data }) => {
